...
void MatrizN::getPrint(int **mat, int tamMat, int *sumF, int totalF, int *sumC, int totalC, int *sumDiagP, int totalDiagP,
    ...
    for (int i = 0; i < tamMat; ++i) {
        cout << "Total Fila " << (i + 1) << ": [" << sumF[i] << "] \n";
    }
    cout << endl;

    cout << "Las Columnas suman \n";
    for (int i = 0; i < tamMat; ++i) {
        cout << "Total Columna " << (i + 1) << ": [" << sumC[i] << "] \n";
    }
    cout << endl;

    cout << endl;
    cout << "El total de las diagonal principal es: " << totalDiagP << endl;
    cout << endl;

    cout << endl;
    cout << "El total de las diagonal secundaria es: " << totalDiagS << endl;
    cout << endl;

    bool isMagic = false;
    for (int i = 0; i < tamMat; ++i) {
        if(totalDiagP == sumF[i] && totalDiagP == sumC[i] && totalDiagP == totalDiagS){
            isMagic = true;
        }
    }
    if (isMagic == true){
